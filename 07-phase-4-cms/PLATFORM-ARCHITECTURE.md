...
`why-us` — typed editorial trust binding: renderer `why-us`, rendererVersion `1`, schemaVersion `1`; config содержит `eyebrow`, `title`, `description`, ordered `facts` (1–8 stable semantic IDs с `number`, `title`, `description`) и `team` (`label`, `title`, `description`). Materialization и public source проверяют итоговый конфиг после inheritance; ошибка или неподдержанная версия блокирует публикацию (`CMS_WHY_US_SECTION_INVALID`), invalid public response даёт `503`. Главная и catch-all рендерят только release snapshot; operational price/availability/capacity в конфиг не входят. Командная фотография пока остаётся approved presentation asset и не является CMS media reference.

`homepage-section` — shared typed editorial binding для стандартных секций главной: `eyebrow`, `title`, `description`, optional internal `action`, release order и visibility. `SITE_CONTENT_SOURCE=fixture` разрешает статическую главную только в Astro dev (noindex); production игнорирует флаг. Заголовки и composition snapshot главной уже проходят CMS delivery. Для house detail добавлен отдельный `public.house-summary.v1`: CMS route и safe operational projection должны совпасть по path/title/release; capacity берётся из одной fixed Resource, цена — из active rate plans, а отсутствие price authority остаётся `request_only`. Для campground detail добавлен `public.campground-summary.v1`: `owned_tent` и `own_tent_pitch` проходят разные typed capacity/membership guards, а public route не вычисляет shared availability. Для add-on detail используется существующий `public.addon-summary.v1`: CMS `addon_detail` получает operational identity из release dependency, а ContentSource сверяет его с тем же release/title перед SSR. Для program detail добавлен `public.program-summary.v1`: CMS `program_detail` закрепляет CatalogOffering/template за release, а public route показывает только duration, participant limits, next open occurrence и conservative price/readiness; registrations и quote acceptance остаются CRM-owned. Для event-service detail добавлен `public.event-service-summary.v1`: CMS `event_detail` закрепляет CatalogOffering/EventServiceTemplate за release, а public route показывает только format, duration, guest bounds и request-only readiness; customer Event, PII, resource selections и event-order quote не пересекают public boundary. `/blog` и `/resources/[slug]` требуют отдельных typed public projections, поэтому их статическое содержимое не считается production-ready data delivery.
