...
1. Ввести dossier «Формат мероприятия» на `EventServiceTemplate` с typed interval/package quote; затем подключить add-ons и lifecycle acceptance к фактическому `/events/:id`, который остаётся customer order и не получает public/CMS полей.
