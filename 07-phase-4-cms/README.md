...
| P4.5 public | Release-pinned add-on, venue, house и campground listing/detail projections; ContentSource и 404/503/release consistency; standalone SSR/asset delivery gate; typed editorial CMS→publication→SSR bindings всех стандартных секций главной; CMS-driven `/houses/*` и `/campgrounds/*` routes | Typed safe public resolvers для operational card collections и program/event-service routes; route-by-route migration |
...
1. P4.5F route-by-route migration: homepage, house и campground vertical route gates закрыты; следующим переводить addon → venue → program → event service по мере готовности safe resolver.
