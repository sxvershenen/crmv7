...
| P4.5 public | Release-pinned add-on, venue, house, campground и program listing/detail projections; ContentSource и 404/503/release consistency; standalone SSR/asset delivery gate; typed editorial CMS→publication→SSR bindings всех стандартных секций главной; CMS-driven `/houses/*`, `/campgrounds/*`, `/addons/*`, `/venues/*` и `/programs/*` routes | Typed safe public resolvers для event-service routes; route-by-route migration |
...
1. P4.5F route-by-route migration: homepage, house, campground, addon, venue и program vertical route gates закрыты; следующим переводить event service по мере готовности safe resolver.
