...
| P4.5 public | Release-pinned add-on, venue и house listing/detail projections; ContentSource и 404/503/release consistency; standalone SSR/asset delivery gate; typed editorial CMS→publication→SSR bindings всех стандартных секций главной; CMS-driven `/houses/*` route | Typed safe public resolvers для operational card collections и campground/program/event-service routes; route-by-route migration |
...
1. P4.5F route-by-route migration: homepage и house vertical route gates закрыты; следующим переводить campground, затем addon → venue → program → event service по мере готовности safe resolver.
