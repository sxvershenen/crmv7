...
export function EventServiceOfferingEditorPage({ repository = eventServiceRepository }: { repository?: EventServiceRepository }) {
  ...
  if (loading) return <div aria-label="Загрузка категории мероприятия" className="mx-auto max-w-[1480px] space-y-4 p-5" role="status"><div className="h-8 w-64 animate-pulse rounded bg-muted" /><div className="h-72 animate-pulse rounded-xl border bg-muted/20" /></div>
  if (error) return <div className="mx-auto max-w-[960px] p-5"><div className="rounded-xl border bg-background"><PageState actionLabel="Повторить" icon={IconAlertTriangle} onAction={() => void load()} title="Категория не открылась" tone="danger">{error}</PageState></div></div>
