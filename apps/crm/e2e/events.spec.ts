...
test("program categories and the event category dossier keep responsive route-driven chrome", async ({ page, isMobile }) => {
  ...
  await page.goto("/events/categories/new")
  await page.getByRole("textbox", { name: "Название" }).fill("Категория E2E")
  await page.getByRole("textbox", { name: "Код" }).fill("event_category_e2e")
  await page.getByRole("textbox", { name: "Внутренняя заметка" }).fill("Только для команды")
  await page.getByRole("combobox", { name: "Иконка" }).click()
  await page.getByRole("option", { name: "Здание" }).click()
  await page.getByRole("combobox", { name: "Цвет" }).click()
  await page.getByRole("option", { name: "Фиолетовый" }).click()
  await page.getByRole("button", { name: "Создать категорию" }).click()
  await expect(page).toHaveURL(/\/events\/categories\/[0-9a-f-]+$/)
  await expect(page.locator('[data-slot="editor-frame"]')).toBeVisible()
  await expect(page.getByRole("textbox", { name: "Название" })).toHaveValue("Категория E2E")
  await expect(page.getByRole("textbox", { name: "Внутренняя заметка" })).toHaveValue("Только для команды")
  await expect(page.getByRole("combobox", { name: "Иконка" })).toContainText("Здание")
  await expect(page.getByRole("combobox", { name: "Цвет" })).toContainText("Фиолетовый")
