...
test("priced program registration uses a current server quote on desktop and mobile", async ({ page }) => {
  await page.goto("/programs/registrations/5009?tab=payment")
  await expect(page.getByText("Цену фиксирует серверный расчёт")).toBeVisible()
  await expect(page.getByLabel("Стоимость")).toHaveCount(0)
  const confirm = page.getByRole("button", { name: "Подтвердить по расчёту" })
  ...
  await expect(page.getByText("Рассчитанная стоимость")).toBeVisible()
  await expect(confirm).toBeEnabled()
  await page.getByLabel("Завтрак").fill("2")
  ...
  await expect(page.getByLabel("Завтрак")).toHaveCount(0)
  await expect.poll(() => page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true)
})
