...
import {
  ...
  PublicHouseDetailQuerySchema,
  PublicHouseListResponseSchema,
  PublicHouseProjectionPinSchema,
  PublicHouseSummarySchema,
  PublicReleasePageContentSchema,
  ReleaseDependencyRefSchema,
  IdSchema,
  type PublicHouseDetailQuery,
  type PublicHouseListQuery,
  type PublicHouseListResponse,
  type PublicHouseSummary,
} from "@crm/contracts"
import { PriceRuleEntity, RatePlanEntity } from "@crm/db"

import { PUBLIC_HOUSE_PROJECTION_CONTRACT, publicHouseProjectionPinHash } from "../offerings/public-house-projection.js"
...
export type PublicHouseProjectionDocument<T> = Readonly<{ data: T; etag: string; cacheTags: string[] }>

@Injectable()
export class PublicHouseOfferingService {
  constructor(@Inject(DataSource) private readonly dataSource: DataSource) {}

  async list(query: PublicHouseListQuery): Promise<PublicHouseProjectionDocument<PublicHouseListResponse>> {
    ...
    const data = PublicHouseListResponseSchema.parse({
      items: summaries,
    ...
    return this.document(data, [`cms-release:${release.id}`, "public-offering-kind:house", "public-offering-collection"])
  }

  async detail(query: PublicHouseDetailQuery): Promise<PublicHouseProjectionDocument<PublicHouseSummary>> {
    PublicHouseDetailQuerySchema.parse(query)
  ...
  private async rows(paths: string[] | null, cursor: Cursor | null, limit: number): Promise<ProjectionRow[]> {
    return this.dataSource.query(`
      ...
      JOIN catalog_offerings offering ON offering.id = source.source_id
        AND offering.kind = 'house' AND offering.state = 'active' AND offering.archived_at IS NULL
      ...
      WHERE active.singleton_key = 'public'
        AND (item.path LIKE '/houses/%' OR item.path LIKE '/domiki/%')
  ...
  private async summaries(rows: ProjectionRow[]): Promise<PublicHouseSummary[]> {
  ...
  private summary(row: ProjectionRow, plans: RatePlanEntity[], rulesByPlan: Map<string, PriceRuleEntity[]>): PublicHouseSummary {
    ...
    const pin = PublicHouseProjectionPinSchema.parse({ contract: PUBLIC_HOUSE_PROJECTION_CONTRACT, offeringId: row.offeringId, kind: "house", nodeId: row.nodeId, profileRevisionId: row.revisionId })
    ...
    if (!dependency || dependency.version !== pin.contract || dependency.contentHash !== publicHouseProjectionPinHash(pin)) throw this.invalidProjection()
  ...
  private document<T>(data: T, cacheTags: string[]): PublicHouseProjectionDocument<T> {
  ...
  private notFound() { return new NotFoundException({ code: "NOT_FOUND", message: "Опубликованный домик не найден" }) }
  private invalidProjection() { return new ServiceUnavailableException({ code: "PUBLIC_PROJECTION_INVALID", message: "Публичная проекция домика требует повторной публикации" }) }
}
