...
const content = { kind: "resource_detail", path: "/venues/meadow", title: "Поляна", summary: "Открытая площадка", hero: null, sections: [], seo: { title: "Поляна", description: "Площадка", indexPolicy: "index_follow", canonical: { mode: "self" }, structuredData: [] } }
...
function subject(projectionRows = [row()]) {
  ...
  const repositories = new Map<unknown, { findBy: ReturnType<typeof vi.fn> }>([
    [RatePlanEntity, { findBy: vi.fn().mockResolvedValue([{ id: ratePlanId, priceBookId, pricingBasis: "per_hour", baseAmountMinor: 2500 }]) }],
    [PriceRuleEntity, { findBy: vi.fn().mockResolvedValue([]) }],
  ])
  return { service: new PublicVenueOfferingService({ query, getRepository: vi.fn((entity) => repositories.get(entity)) } as never), query }
}

describe("PublicVenueOfferingService", () => {
  it("serves a release-pinned venue without interval availability counts", async () => {
    const result = await subject().service.detail(offeringId)
    expect(result.data).toMatchObject({ kind: "venue", price: { mode: "from" }, fulfillment: { capacityTotal: 40, availabilityMode: "resource" }, capacity: null })
    expect(result.cacheTags).toContain("public-offering-kind:venue")
  })
