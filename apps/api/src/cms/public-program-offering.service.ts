...
import {
  ...
  type PublicProgramListQuery,
  type PublicProgramListResponse,
  type PublicProgramSummary,
} from "@crm/contracts"
import { PriceRuleEntity, RatePlanEntity } from "@crm/db"

import { PUBLIC_PROGRAM_PROJECTION_CONTRACT, publicProgramProjectionPinHash } from "../offerings/public-program-projection.js"
...
type ProjectionRow = {
  ...
  nextOccurrenceParticipantLimit: number | null
  nextOccurrenceRegistrationLimit: number | null
  titleKey: string
}

type Cursor = { title: string; id: string }
export type PublicProgramProjectionDocument<T> = Readonly<{ data: T; etag: string; cacheTags: string[] }>

@Injectable()
export class PublicProgramOfferingService {
  constructor(@Inject(DataSource) private readonly dataSource: DataSource) {}

  async list(query: PublicProgramListQuery): Promise<PublicProgramProjectionDocument<PublicProgramListResponse>> {
    ...
    const data = PublicProgramListResponseSchema.parse({
      items: summaries,
    ...
    return this.document(data, [`cms-release:${release.id}`, "public-offering-kind:program", "public-offering-collection"])
  }

  async detail(offeringId: string): Promise<PublicProgramProjectionDocument<PublicProgramSummary>> {
    PublicProgramSummaryParamsSchema.parse({ offeringId })
    ...
    return this.document(summary, [`cms-release:${row.releaseId}`, `cms-node:${row.nodeId}`, `public-offering:${row.offeringId}`, "public-offering-kind:program", "public-offering-collection"])
  }
  ...
  private async rows(cursor: Cursor | null, limit: number, offeringId: string | null = null): Promise<ProjectionRow[]> {
    return this.dataSource.query(`
      SELECT release.id AS "releaseId", release.created_at AS "releaseCreatedAt", release.published_at AS "releasePublishedAt",
        ...
        price_book.updated_at AS "priceBookUpdatedAt", template.id AS "templateId", template.version AS "templateVersion",
        template.updated_at AS "templateUpdatedAt", template.duration_minutes AS "durationMinutes",
        template.minimum_participants AS "minimumParticipants", template.participant_limit AS "participantLimit",
        next_occurrence.starts_at AS "nextOccurrenceStartsAt", next_occurrence.ends_at AS "nextOccurrenceEndsAt",
        next_occurrence.participant_limit AS "nextOccurrenceParticipantLimit", next_occurrence.registration_limit AS "nextOccurrenceRegistrationLimit",
      ...
      JOIN cms_nodes node ON node.id = item.node_id AND node.kind = 'program_detail' AND node.status = 'active' AND node.archived_at IS NULL
      ...
      JOIN catalog_offerings offering ON offering.id = source.source_id AND offering.kind = 'program' AND offering.state = 'active' AND offering.archived_at IS NULL
      JOIN offering_bindings binding ON binding.offering_id = offering.id AND binding.role = 'primary' AND binding.program_template_id IS NOT NULL AND binding.archived_at IS NULL
      JOIN program_templates template ON template.id = binding.program_template_id AND template.publication = 'published' AND template.archived_at IS NULL
      ...
      LEFT JOIN LATERAL (
        SELECT occurrence.starts_at, occurrence.ends_at, occurrence.participant_limit, occurrence.registration_limit
        FROM program_occurrences occurrence
        WHERE occurrence.template_id = template.id AND occurrence.status = 'open' AND occurrence.archived_at IS NULL AND occurrence.starts_at >= NOW()
  ...
  private async summaries(rows: ProjectionRow[]): Promise<PublicProgramSummary[]> {
  ...
  private summary(row: ProjectionRow, plans: RatePlanEntity[], rulesByPlan: Map<string, PriceRuleEntity[]>): PublicProgramSummary {
    ...
    if (!dependency || dependency.version !== pin.contract || dependency.contentHash !== publicProgramProjectionPinHash(pin)) throw this.invalidProjection()
    ...
    const nextOccurrence = row.nextOccurrenceStartsAt && row.nextOccurrenceEndsAt && row.nextOccurrenceParticipantLimit && row.nextOccurrenceRegistrationLimit ? {
      startsAt: row.nextOccurrenceStartsAt.toISOString(), endsAt: row.nextOccurrenceEndsAt.toISOString(), participantLimit: row.nextOccurrenceParticipantLimit, registrationLimit: row.nextOccurrenceRegistrationLimit,
    } : null
    ...
    return PublicProgramSummarySchema.parse({
      ...
      asOf,
      fulfillment: { durationMinutes: row.durationMinutes, minimumParticipants: row.minimumParticipants, participantLimit: row.participantLimit, availabilityMode, nextOccurrence },
    })
  }
  ...
  private document<T>(data: T, cacheTags: string[]): PublicProgramProjectionDocument<T> { return { data, etag: `"${createHash("sha256").update(stableStringify(data)).digest("hex")}"`, cacheTags } }
  ...
  private decodeCursor(value: string): Cursor {
    try {
      ...
      PublicProgramSummaryParamsSchema.parse({ offeringId: parsed.id })
  ...
  private notFound() { return new NotFoundException({ code: "NOT_FOUND", message: "Опубликованная программа не найдена" }) }
  private invalidProjection() { return new ServiceUnavailableException({ code: "PUBLIC_PROJECTION_INVALID", message: "Публичная проекция программы требует повторной публикации" }) }
}
