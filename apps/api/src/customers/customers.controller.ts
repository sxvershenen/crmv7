...
import { CustomerArchiveSchema, CustomerAssignSelfSchema, CustomerCreateSchema, CustomerListQuerySchema, CustomerUpdateSchema, type CustomerArchive, type CustomerAssignSelf, type CustomerCreate, type CustomerListQuery, type CustomerUpdate } from "@crm/contracts"
...
export class CustomersController {
  ...
  @Post(":id/assign-self") @RequireCapabilities("canAssign") assignSelf(@Param("id") id: string, @Body(new ZodValidationPipe(CustomerAssignSelfSchema)) input: CustomerAssignSelf, @Req() request: AuthenticatedRequest) { return this.customers.assignSelf(id, input.version, request.sessionUser!, request.requestId) }
