...
## D-084 — ProgramRegistration принимает только occurrence-bound quote

Acceptance-ready `program_registration` quote привязан к exact ProgramTemplate/ProgramOccurrence/offering versions, датам, participants, currency, tariff и assignment/quantity supported add-ons. `template_preview` не принимается, quote сам не резервирует capacity, а expired или stale context требует requote. V1 разрешает только `quantity_service` и `person_service`; request-only, scheduled-resource и другие targets остаются fail-closed.
