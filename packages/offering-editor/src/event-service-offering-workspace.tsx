...
type TemplateDraft = Pick<EventServiceOfferingDossier["template"], "format" | "defaultDurationMinutes" | "minimumGuests" | "maximumGuests" | "preparationBeforeMinutes" | "preparationAfterMinutes">
...
function EventServiceTermsEditor({ data, gateway, createCommandMeta, onReload, onSaveActionChange, onSaveState }: { data: EventServiceEditorData; gateway: Pick<EventServiceOfferingGateway, "updateTemplate">; createCommandMeta: EventServiceOfferingWorkspaceProps["createTemplateCommandMeta"]; onReload: () => Promise<void>; onSaveActionChange: (action: (() => void) | null) => void; onSaveState: (state: SaveState, detail: string) => void }) {
  ...
  const initial = useMemo<TemplateDraft>(() => ({ format: template.format, defaultDurationMinutes: template.defaultDurationMinutes, minimumGuests: template.minimumGuests, maximumGuests: template.maximumGuests, preparationBeforeMinutes: template.preparationBeforeMinutes, preparationAfterMinutes: template.preparationAfterMinutes }), [template])
  ...
  const save = async () => {
    ...
    try {
      const body = command.current?.body ?? { ...draft, ...createCommandMeta(), expectedSubjectVersion: template.version }
