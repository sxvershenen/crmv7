...
export function EventServiceOfferingWorkspace({ data, gateway, createTemplateCommandMeta, createPricingCommandMeta, createPreviewCommandMeta, initialTab = "terms", onBack, onReload, onNavigationGuardChange, onTabChange, editorialHref }: EventServiceOfferingWorkspaceProps) {
  ...
  return <EditorFrame footerActions={footer} navigation={navigation} saveDetail={activeDetail} saveState={activeState} sidebar={<EventServiceSidebar data={data} {...(editorialHref ? { editorialHref } : {})} />}><div aria-hidden={tab !== "terms"} hidden={tab !== "terms"}><EventServiceTermsEditor data={data} createCommandMeta={createTemplateCommandMeta} gateway={gateway} onReload={onReload} onSaveActionChange={registerTemplateAction} onSaveState={(state, detail) => { setTemplateState(state); setTemplateDetail(detail) }} /></div><div aria-hidden={tab !== "pricing"} hidden={tab !== "pricing"}><EventServicePricingEditor data={data} createCommandMeta={createPricingCommandMeta} gateway={gateway} onReload={onReload} onSaveActionChange={registerPricingAction} onSaveState={(state, detail) => { setPricingState(state); setPricingDetail(detail) }} /></div><div aria-hidden={tab !== "preview"} hidden={tab !== "preview"}><EventServiceQuotePreview data={data} createCommandMeta={createPreviewCommandMeta} gateway={gateway} /></div></EditorFrame>
}
